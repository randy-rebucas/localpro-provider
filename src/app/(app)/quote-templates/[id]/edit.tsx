...
export default function EditTemplateScreen() {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { name: '', laborCost: '', materialsCost: '', timeline: '', notes: '', milestones: [] },
  });
  ...
  useEffect(() => {
    if (template) {
      reset({
        ...
        materialsCost: String(template.materialsCost),
        timeline:      template.timeline,
        notes:         template.notes ?? '',
        milestones:    template.milestones?.map((m) => ({ title: m.title, amount: String(m.amount) })) ?? [],
      });
    }
  }, [template, reset]);
  ...
  const mutation = useMutation({
    ...
    onError: () => Alert.alert('Error', 'Could not update template. Please try again.'),
  });
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={[styles.header, { borderBottomColor: theme.backgroundElement }]}>
          ...
          <Text style={[styles.headerTitle, { color: theme.text }]}>Edit Template</Text>
          <View style={{ width: 32 }} />
        </View>
